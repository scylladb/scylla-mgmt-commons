...
type retryableTransport struct {
	...
	poolSize          int
	statusCodeOfFunc  StatusCodeOfFunc
	logger            log.Logger
}
...
type StatusCodeOfFunc func(err error) int

// Retryable wraps parent and adds retry capabilities.
func Retryable(transport runtime.ClientTransport, config, interactiveConfig BackoffConfig, poolSize int, statusCodeOfFunc StatusCodeOfFunc, logger log.Logger) runtime.ClientTransport {
...
func (o *retryableOperation) shouldRetry(err error) bool {
	...
	c := o.retryableTransport.statusCodeOfFunc(err)
	if c == 0 || (c >= 500 && c != 501) {
		return true
	}
