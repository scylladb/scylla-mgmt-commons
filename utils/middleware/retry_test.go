...
func fastRetry(config *scyllaclienttest.Config) {
	config.BackoffConfig.WaitMin = 50 * time.Millisecond
...
func TestRetryHostPool(t *testing.T) {
	...
	t.Run("error", func(t *testing.T) {
		...
		multiHost := func(config *scyllaclienttest.Config) {
			config.Hosts = []string{
				"127.0.0.1",
				"127.0.0.2",
				"127.0.0.3",
...
func TestNoRetry(t *testing.T) {
	...
	ctx := middleware.NoRetry(context.Background())
...
func TestRetryTimeout(t *testing.T) {
	...
	shortTimeout := func(config *scyllaclienttest.Config) {
		config.Transport = middleware.Timeout(config.Transport, 30*time.Millisecond)
	}
	...
	t.Run("interactive", func(t *testing.T) {
		_, err := client.NodeInfo(middleware.Interactive(context.Background()), host)
		if err == nil {
			t.Fatal("NodeInfo() expected error")
		}
	})
	t.Run("custom timeout", func(t *testing.T) {
		_, err := client.NodeInfo(middleware.CustomTimeout(context.Background(), 5*time.Millisecond), host)
		if err != nil {
			t.Fatal("NodeInfo() error", err)
		}
	})
